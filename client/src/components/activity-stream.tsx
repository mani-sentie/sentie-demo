...
export function ActivityStream({ activities, emptyMessage }: ActivityStreamProps) {
...
function ActivityItem({ activity, isNew }: { activity: Activity; isNew: boolean }) {
  ...
  return (
    <div 
      className={`relative flex gap-4 p-4 rounded-lg border transition-all duration-500 ${
        isNew ? 'bg-primary/5 border-primary/30 animate-in fade-in slide-in-from-top-2' : 'bg-card border-card-border'
      }`}
